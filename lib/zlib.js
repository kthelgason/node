...
Zlib.prototype._processChunk = function(chunk, flushFlag, callback) {
  ...
  const handleWrite = () => {
    const writeFunc = async ? this._handle.write : this._handle.writeSync;
    const result =  writeFunc.call(this._handle,
                                   flushFlag,
                                   chunk,
                                   inOff,
                                   availInBefore,
                                   ...
                                   this._offset,
                                   availOutBefore);
    nRead += result ? availOutBefore - result[1] : 0;
    return result;
  };

  const createWriteRequest = () => {
    const req = handleWrite();
    req.callback = after; // this same function
    req.buffer = chunk;
  };

  const after = (availInAfter, availOutAfter, buffers) => {
    if (this._hadError)
      return false;
    ...
    if (have > 0) {
      const out = this._buffer.slice(this._offset, this._offset + have);
      this._offset += have;
    ...
    // Exhausted the output buffer, or used all the input.
    // Create a new one.
  ...
  if (!async) {
    ...
    } while (after(res[0], res[1], buffers));
